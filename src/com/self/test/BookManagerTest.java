...
public class BookManagerTest {
	...
	public static void main(String[] args) {
		...
		try {
			service.insertBook(new Magazine(201, "ELLE", "작가", "출판사10", 5000.0, new Date(2025,03)));
		} catch(DuplicateIsbnException e) {
			System.out.println(e.getMessage());
		}
		try {
		...
		try {
			service.updateBook(new Magazine(999, "ELLE", "작가", "출판사11", 5000.0, new Date(2025,01)));
		} catch(RecordNotFoundException e) {
			System.out.println(e.getMessage());
		}
...
//		System.out.println("가장 인기있는 소설 장르 : "+service.getPopularGenre());
		...
		ArrayList<Book> map1 = service.getAllBook();
		Collections.sort(map1, new Comparator<Book>() {
			@Override
		...
		try {
			ArrayList<Book> map2 = service.magazineOfThisYearInfo(2024);
		}catch(MagazineNotFoundException e) {
			System.out.println(e.getMessage());
		}
		try {
			ArrayList<Book> map3 = service.magazineOfThisYearInfo(2025);
			Collections.sort(map3, new Comparator<Book>() {
				@Override
				public int compare(Book o1, Book o2) {
